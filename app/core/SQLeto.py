...
class SQLeto:
    ...
    @classmethod
    def connect_database(self, engine) -> sql.engine.Connection:

        return engine.connect()

    @classmethod
    def execute_DQL(self, query: str) -> pd.DataFrame:
    ...
    @classmethod
    def get_dataframes_to_upload(self, path: str):
    ...
    @classmethod
    def create_type_column(self, data: pd.DataFrame) -> dict:
    ...
    def send_dataframes_to_tables(
        self, list_names_order: list, if_exists_: str
    ) -> None:
        ...
        for name in names:
            ...
            globals()[f"df_{name}"].to_sql(
                name, self.create_engine(), if_exists=if_exists_, index=False
            )
            time.sleep(0.8)
